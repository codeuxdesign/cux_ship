...
**CI runs a sixth step and you never run it locally. Do not pass `--docs`.**
It roughly doubles the loop to re-ask a question only a dartdoc edit can
change, and CI asks it on every push, so a broken `[reference]` costs a red
board rather than a wrong release. The flag exists for CI's own
`--docs-only`; `tool/check.sh`'s header carries why, and why the earlier rule —
*ask for it when you have touched a doc comment* — was replaced by *never*.
