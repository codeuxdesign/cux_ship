...
/// Credentials for a **team** API key, as created under Users and Access >
/// Integrations. An individual key would put the key id in `sub` instead;
/// nothing in this repository uses one.
class AscCredentials {
  ...
  static AscCredentials? fromEnvironment() {
    ...
    final issuerId = Platform.environment['APPLE_API_ISSUER_ID'];
    final keyPath = Platform.environment['APPLE_API_PRIVATE_KEY_PATH'];
    if (keyId == null || issuerId == null || keyPath == null) {
      return null;
    }
    if (keyId.isEmpty || issuerId.isEmpty || keyPath.isEmpty) {
      return null;
    }
...
class AscClient {
  ...
  ///
  /// `aud` is set in the payload directly rather than through the library's
  /// `audience` field, which serialises to a JSON array. The spec permits that
  /// and Apple does not accept it.
